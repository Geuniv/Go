...
// 파일 읽기 쓰기
// os.OpenFile 함수를 사용하여 읽기/쓰기 모드로 파일을 연 뒤 파일을 읽고 씀

func main() {
	file, err := os.OpenFile(
		"hello.txt",
		os.O_CREATE|os.O_TRUNC, // 파일이 없으면 생성, 읽기/쓰기, 파일을 연 뒤 내용삭제
	...
	file.Seek(0, os.SEEK_SET)          // Write 함수로 인해 파일 읽기/쓰기 취가치 이동했으므로
	// file.Seek 함수로 파일의 맨 처음 ( 0 ) 으로 이동

	n, err = file.Read(data) // 파일의 내용을 읽어서 바이스 슬라이스에 저장
	if err != nil {
		fmt.Println(err)
		return
	}
